...
class DerStandardCrawler(SitemapSpider):
        ...
        if story_item.get('story_posting_count'):
            ...
            for page_number in range(1, story_posting_pages + 1):
                story_posting_next_page_url = 'https://apps.derstandard.at/forum/postings/Index/1/' + str(story_item.get('story_id')) + '?ForumKey.ForumKeyId=' + str(story_item.get(
                    'story_id')) + '&ForumKey.ForumKeyType=1&CurrentPage=' + str(page_number) + '&Filter.SelectedFilterType=0&SelectedSortType=0&SelectedPostingId=&X-Requested-With=XMLHttpRequest'
                yield response.follow(url=story_posting_next_page_url,
                                      headers={
                                          'Accept': '*/*',
                                      ...
                                      cb_kwargs=dict(posting_story_id=response.url, posting_page_number=page_number))
    ...
    def parse_postings(self, response, posting_story_id, posting_page_number):
        ...
        posting_item_loader.add_value(
            'posting_story_id', posting_story_id)
        posting_item_loader.add_value(
            'posting_page_number', posting_page_number)
        posting_item_loader.add_xpath(
            'posting_content', '//div[@id="postinglist"]')
