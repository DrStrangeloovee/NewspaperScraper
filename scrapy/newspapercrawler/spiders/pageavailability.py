...
from newspapercrawler.items import MycrawlerItem
...
class PageavailabilitySpider(CrawlSpider):
    ...
    start_urls = ["http://derstandard.at/"]
    custom_settings = {"LOG_FILE": "logs/pageavailability.log", "LOG_LEVEL": "INFO"}

    rules = (
        Rule(
            LinkExtractor(allow=("/story/"), tags="a", attrs="href", unique=True),
            callback="parse_item",
            follow=True,
        ),
    )
